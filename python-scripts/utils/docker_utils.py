    ...
    docker_compose = {
        ...
        'substra_tools': {
            'setup': {
                ...
                'command': f'/bin/bash -c "set -o pipefail;python3 /scripts/setup.py 2>&1'
                           f' | tee {substra_path}/data/log/setup-{org["name"]}.log"',
                'environment': [
                ...
                'volumes': [
                    ...
                    f'{substra_path}/conf/{org["name"]}/fabric-ca-client-config.yaml:{FABRIC_CA_CLIENT_HOME}/fabric-ca-client-config.yaml',  # noqa
            ...
            'run': {
                ...
                'image': 'substra/substra-ca-tools',
                'command': f'/bin/bash -c "set -o pipefail;sleep 3;python3 /scripts/run.py 2>&1'
                           f' | tee {substra_path}/data/log/run-{org["name"]}.log"',
                'environment': [
                    'GOPATH=/opt/gopath',
                    f'FABRIC_CFG_PATH={FABRIC_CFG_PATH}',
                    f'ORG={org["name"]}',
                ...
                'volumes': [
                    ...
                    f'{substra_path}/conf/{conf_orderer["name"]}/{orderer["name"]}/:{substra_path}/conf/{conf_orderer["name"]}/{orderer["name"]}/',  # noqa
    ...
    docker_compose = {
        ...
        'substra_tools': {
            'setup': {
                'container_name': f'setup-{org["name"]}',
                'image': 'substra/substra-ca-tools',
                'command': f'/bin/bash -c "set -o pipefail;python3 /scripts/setup.py 2>&1 | tee {substra_path}/data/log/setup-{org["name"]}.log"',  # noqa
                'environment': [
                    f'FABRIC_CA_HOME={FABRIC_CA_HOME}',
                    f'FABRIC_CFG_PATH={FABRIC_CFG_PATH}',
                    f'FABRIC_CA_CLIENT_HOME={FABRIC_CA_CLIENT_HOME}'
    ...
    for _, orderer in enumerate(org['orderers']):
        svc = {
            ...
            'volumes': [
                ...
                # tls server files
                f"{orderer['tls']['dir']['external']}/{orderer['tls']['server']['dir']}:{orderer['tls']['dir'][ 'internal']}/{orderer['tls']['server']['dir']}",  # noqa

                # conf files
                f"{substra_path}/data/configtx-{org['name']}.yaml:{org['core_dir']['internal']}/configtx.yaml",
                f"{substra_path}/conf/{org['name']}/{orderer['name']}/core.yaml:{org['core_dir']['internal']}/core.yaml",  # noqa
                f"{substra_path}/conf/{org['name']}/{orderer['name']}/orderer.yaml:{org['core_dir']['internal']}/orderer.yaml",  # noqa
    ...
    COMPOSITION = {
        'services': {
            'fixtures': {
                ...
                'image': 'substra/substra-ca-tools',
                'command': f'/bin/bash -c "set -o pipefail;python3 /scripts/{fixtures_path} 2>&1'
                           f' | tee {substra_path}/data/log/fixtures.log"',
                'environment': [
                    f'FABRIC_CA_HOME={FABRIC_CA_HOME}',
                    f'FABRIC_CFG_PATH={FABRIC_CFG_PATH}',
                    f'FABRIC_CA_CLIENT_HOME={FABRIC_CA_CLIENT_HOME}',
    ...
    COMPOSITION = {
        'services': {
            'revoke': {
                ...
                'image': 'substra/substra-ca-tools',
                'command': f'/bin/bash -c "set -o pipefail;python3 /scripts/revoke.py 2>&1'
                           f' | tee {substra_path}/data/log/revoke.log"',
                'environment': [
                    f'FABRIC_CA_HOME={FABRIC_CA_HOME}',
                    f'FABRIC_CFG_PATH={FABRIC_CFG_PATH}',
                    f'FABRIC_CA_CLIENT_HOME={FABRIC_CA_CLIENT_HOME}',
